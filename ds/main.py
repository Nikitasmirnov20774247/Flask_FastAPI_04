...
def download_img(url, dir_path=PATH):
    ...
    print(f'Время загрузки файла: {filename} = {end_time:.2f} сек')
...
def parse():
    ...
    parser.add_argument('-u', '--urls', default=images, nargs='+', 
                        type=str, help='Список URL-адресов')
...
if __name__ == '__main__':
    ...
    print(f'\nЗагрузка {len(urls)} изображений через мультипотоки')
    download_img_thread(urls)

    print(f'\nЗагрузка {len(urls)} изображений через мультипроцессы')
